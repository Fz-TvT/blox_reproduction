...
def main(args):
    ...
    if args.simulate:
        while True:
            ...
            args.scheduler = args.scheduler_name    # <- 新增：同步两个字段
            ...
            if args.scheduler_name == "Las":
                ...
                scheduling_policy = schedulers.Optimus(args,job_state)
            ...
            while True:
                ...
                new_jobs = blox_mgr.pop_wait_queue(args.simulate) ###增加新作业
...
def parse_args(parser):
    ...
    parser.add_argument(
        "--round-duration", type=int, default=5000, help="Round duration in seconds"
    )
    parser.add_argument(
        "--start-id-track", type=int, default=0, help="Starting ID to track"
    )
    parser.add_argument(
        "--stop-id-track", type=int, default=1000, help="Stop ID to track"
    )
